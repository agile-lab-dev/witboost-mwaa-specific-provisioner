...
export NAME=mwaa-specific-provisioner
export IMAGE_NAME=registry.gitlab.com/agilefactory/witboost.mesh/provisioning/sandbox/witboost.mesh.provisioning.sandbox.mwaaspecificprovisioner
...
helm upgrade --atomic --install -n ${NAMESPACE} --wait --timeout 120s --debug --description "${COMMIT}" -f values.yaml.dec ${NAME} ./helm \
--set name="${NAME}" \
